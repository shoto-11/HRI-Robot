...
\begin{enumerate}[leftmargin=2em]
  ...
  \item 時間系の勝者（比較時のみ $2R_p$）と近接の重み付き和で危険度スコア $R \in [0,1]$ を計算（表示は $d\le D_\text{max}$ のみ）
...
\paragraph{三要因}
人へのPTTC $T_p=d/v_\text{close}$（$v_\text{close}=\max(0,\hat{\mathbf{r}}\cdot\mathbf{v})$；近づかないとき $\infty$），
経路--基準線TTC $T_c=s/\max(v,0.1)$，水平距離 $d$．

\paragraph{統合（時間系の勝者 + 近接）}
\begin{align}
...
\end{align}
$\gamma=0.6$，$w_t=0.85$，$w_d=0.15$。係数 $2$ は勝ち負け判定のみ（スコア本体には掛けない）。
