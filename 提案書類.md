...
| **No-AR** | — | 経路非表示 |
| **Proposed** | PTTC + 経路TTC + 近接（重み付き和） | 危険度連動（本書の対象） |
...
**PTTC（人への閉じ込み）** $T_p$：参加者位置への相対ベクトル $\mathbf{r}$（水平）と AGV 速度 $\mathbf{v}$ から

$$
v_\text{close} = \max(0,\, \hat{\mathbf{r}}\cdot\mathbf{v}), \quad
T_p =
\begin{cases}
...
$$
R = \max\!\Bigl(R_\text{floor},\; \min\!\bigl(1,\; w_p R_p + w_c R_c + w_d R_d\bigr)\Bigr)
$$

| 記号 | 初期値 | 役割 |
|------|--------|------|
| $w_p$ | **0.55** | 人への迫り（PTTC）を主因に |
| $w_c$ | **0.30** | 視線基準線横断（研究の独自性） |
| $w_d$ | **0.15** | 近接は保険（主役にしない） |
| $R_\text{floor}$ | **0.08** | 表示中の最低スコア |

$\max(R_\text{ttc}, R_\text{prox})$ だと「近いが向かない」が勝ちやすい。重み付き和により「遠いが正面から迫る」を相対的に高くし、複合危険も加算できる。
...
| 融合重み | $w_p,w_c,w_d$ | 0.55, 0.30, 0.15 | PTTC 主・path TTC 副・近接は保険 |
...
|------|----------|-------|----------|
| 経路表示 | あり | なし | あり |
| 危険度計算 | なし（$R=1$ 固定） | — | PTTC + 経路TTC + 近接（重み付き和） |
...
2. **表示は距離のみ、スコアは三要因の重み付き和**: 出す／出さないは $d\le D_\text{max}$。危険の強さは PTTC 主・path TTC 副・近接は弱い保険。
