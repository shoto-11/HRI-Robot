...
$\max(R_\text{ttc}, R_\text{prox})$ だと「近いが向かない」が勝ちやすい。重み付き和により「遠いが正面から迫る」を相対的に高くし、複合危険も加算できる。不透明度の下限は `RiskToVisualMapper` の $\alpha_{\min}=0.35$ で担保する（$R_\text{floor}$ は用いない）。
