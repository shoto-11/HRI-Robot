...
| **No-AR** | — | 経路非表示 |
| **Proposed** | PTTC + Crossing TTC（CTTC）+ 近接（重み付き和） | 危険度連動（本書の対象） |
...
**Crossing TTC（CTTC・視線基準線横断）** $T_c$：残り計画経路上を速度 $v_\text{AGV}$ で進んだとき、太線ゾーンとの最初の交差までの時間

$$
T_c = \frac{s}{\max(v_\text{AGV},\, 0.1)}, \quad\text{交差なしまたは }T_c > T_\text{max}\text{ なら }\infty
$$
...
| 記号 | 初期値 | 役割 |
|------|--------|------|
| $w_p$ | **0.55** | 人への迫り（PTTC・相対速度） |
| $w_c$ | **0.30** | 視線基準線横断（Crossing TTC / CTTC） |
| $w_d$ | **0.15** | 近接は保険（主役にしない） |

不透明度の下限は `RiskToVisualMapper` の $\alpha_{\min}=0.35$ で担保する（$R_\text{floor}$ は用いない）。
...
- **形状**: 残り計画経路を AGV 幅の長方形リボン（単一メッシュ）で描画
- **幅**: **1.0 m**（`AgvFootprintM`）一定。先端テーパーなし
- **高さ**: 床面上 **0.22 m** に持ち上げ
...
sequenceDiagram
    ...
    VRC->>Vis: R = wp·Rp + wc·Rc + wd·Rd, 黄〜赤へ
...
| 融合重み | $w_p,w_c,w_d$ | 0.55, 0.30, 0.15 | PTTC 主・CTTC 副・近接は保険 |
...
|------|----------|-------|----------|
| 経路表示 | あり | なし | あり |
| 危険度計算 | なし（$R=1$ 固定） | — | PTTC + CTTC + 近接（重み付き和） |
...
2. **表示は距離のみ、スコアは三要因の重み付き和**: 出す／出さないは $d\le D_\text{max}$。$R=0.55 R_p+0.30 R_c+0.15 R_d$（PTTC は相対速度）。
3. **色・不透明度はともに連続**: 色相と不透明度が $R$ に連動して滑らかに変化し、「だんだん危険」を段階の飛びなく伝える。
4. **2D 交差判定**: Crossing TTC（CTTC）計算は XZ 平面の線分交差。高さ方向の差は直接入らない（床面走行 AGV の簡略化）。
...
| 2026-09-07 | 経路 TTC を Crossing TTC（CTTC）と呼称変更（PTTC との略語衝突回避） |
| 2026-09-07 | PTTC を相対速度 $\mathbf{v}_\text{AGV}-\mathbf{v}_\text{ped}$ に変更 |
| 2026-09-07 | 統合を重み付き和 $R=0.55 R_p+0.30 R_c+0.15 R_d$ に戻す |
